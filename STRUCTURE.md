...
├── preview/                  ☆ 開發預覽頁（Vite，吃 /src/index.scss）
...
| 改文字字型 | `fonts/` + `src/base/_fonts.scss` | 詳見 README「字型管理」章節 |
...
下游專案實際使用時，採 vendored 方式：複製需要的 `src/*.scss` / `assets/symicon.css` / `fonts/` 進自己專案，以本 repo 為唯一真相（詳見 README）。
